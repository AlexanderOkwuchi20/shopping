...
#     categories = Product.objects.filter(category_slug=slug)
...
def contact(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request,"Dear , Your message was sent successfully")
...
def signin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, 'signin unsucessful')
            return redirect('index')
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            newprofile.save()
            login(request, user)
            messages.success(request, f"welcome {user.username}!, Signup succesful")
            send_mail (
                "Thank You",
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_3b1b3aa3407415a3cd2bfd842f2706a62c1d466e'
        curl = 'https://api.paystack.co/transaction/initialize'
        cburl = 'http://16.16.121.129/callback/'
...
def callback(request):
    profile = Profile.objects.filter(user__username= request.user.username)
    trolley = Payment.objects.filter(user__username= request.user.username, paid=False)
    payment = Shopcart.objects.filter(user__username= request.user.username, paid=True)
    ...
    for items in trolley:
        ...
        stock = Product.objects.get(pk=items.products_id)
...
# def index(request):
#     queryset = Model.objectsManager.get,all, filter,exclude,order_by()
#     context = {
#         'queryset': queryset,
#     }
